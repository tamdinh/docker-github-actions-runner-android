...
# Removes rebuildable caches that grow unbounded between jobs while preserving persistent dependency stores.
#
...
# EAS local build temporary working directories if left behind
rm -rf /tmp/eas-build-local-* 2>/dev/null || true
